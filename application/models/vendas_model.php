...
class Vendas_model extends CI_Model {
    ...
    function get($table,$fields,$where='',$perpage=0,$start=0,$one=false,$array='array', $tipo=1){
    ...
    function count($table, $tipo, $faturado = 0){
        $this->db->from($table);
        $this->db->where("tipo", $tipo);
        if($faturado >= 0)
    ...
    public function autoCompleteCliente($q){
        ...
        if($query->num_rows > 0){
            foreach ($query->result_array() as $row){
                $row_set[] = array('label'=>$row['nomeCliente'].' | cpf: '.$row['documento'],'id'=>$row['idClientes']);
            }
            echo json_encode($row_set);
        }
    }
